...
| 小亀基板音声 | J15(FFC **1=L / 3=R**)← RCA白/赤 → **U17** PCM1808 | 16bit/48kHz I2S |
...
## GbE 差動ペアの対応(2026-08-19 検証、v0.9.0 発注後)

**結論: 配線は正しい。** 3つの独立した根拠で一致を確認した。

### ★「どのペアをどこに繋いでも良い」は誤り

以前そう説明していたが間違いだった。実際に自動補正されるのは限られた範囲だけで、
利用者は手半田の試作基板で**リンクアップしない**現象として踏んでいる。

```
オートネゴシエーション(IEEE 802.3 Clause 28)は FLP バーストをペアA でしか流さない
  → 完了するまでペアC/D は一切使われない
  → ペアA が PHY の MDI1/MDI2 に来ていないと**リンク以前に成立しない**

Auto MDI/MDI-X(Clause 40.4.4)が直せるのはクロスケーブル相当の入れ替えだけ。
  A↔B と C↔D が同時に入れ替わるパターンのみ。任意の置換(A↔C など)は対象外
ペア内の極性(+/−)は自動補正される
```

→ **{A,B} は {MDI1,MDI2} に、{C,D} は {MDI3,MDI4} に**乗っていなければならない。

### 検証① HR911130A データシート(datasheets/HR911130A_HanRun_MagJack.pdf の Schematics)

magjack 内部は部品で固定されているので、基板側パッドと RJ45 接点の対応は一意に決まる。
...
| P8 | MDI3+ | J7 = pin 7 | D+ |
| P9 | MDI3− | J8 = pin 8 | D− |
| P1 | chip側センタータップ(4本内部共通) | — | — |
| P10 | Bob-Smith共通(内部 4x75Ω → 1000pF/2kV 経由) | — | — |

### 検証② 発注した基板の実配線(pcbnew から読み出し)

J11 / J12 とも下表どおり。SO-DIMM 側 16本もすべて `sodimm_i5.ato` の宣言と一致。

```
eth*_1_P/N → P2/P3 → ペアA (pin 1,2)   ← オートネゴがここを使う
eth*_2_P/N → P4/P7 → ペアB (pin 3,6)
eth*_3_P/N → P5/P6 → ペアC (pin 4,5)
eth*_4_P/N → P8/P9 → ペアD (pin 7,8)
```

### 検証③ 動作実績のある参照設計との突き合わせ

`kazkojima/colorlight-i5-tips` の **i5ether**(1000BASE-T netboot/nfsroot 実績あり)の
`i5ether.kicad_pcb` からネットリストを抽出し、トランス HX5008 の巻線対応
(chip側 pin N ↔ line側 pin 25−N。センタータップ4箇所 1↔24 / 4↔21 / 7↔18 / 10↔15 が
すべて一致することで確定)まで辿った結果:

```
MDI1P → RJ45 pin 1    MDI1N → pin 2   ペアA
MDI2P → pin 3         MDI2N → pin 6   ペアB
MDI3P → pin 4         MDI3N → pin 5   ペアC
MDI4P → pin 7         MDI4N → pin 8   ペアD
```

**極性まで含めて本基板と完全一致。** `chenzhuoyu/rainbow-board`(同じ magjack を
SO-DIMM 直結で使う独立実装)も同一だった。Bob-Smith の構成(75Ω → 1000pF内蔵 →
シャーシ、外部 4.7nF/2kV でGNDへ)も同じ。

※ i5ether は EXT ボードの P1 ヘッダ(2x15)に挿す設計で、そのヘッダ番号は
  `docs/sodimm-pinmap.html` の P1 番号と**一律 +4 ずれている**(i5ether の J1 pad
  5-12 と 23-30 ↔ 資料の P1·1-8 と 19-26)。オフセットが両ポートで同一なので
  相対関係は保たれており、上記の対応付けに影響しない。本基板は SO-DIMM 直結なので
  EXT の番号自体は無関係。

### ★実装時の注意

**1. 基板に ETH1/ETH2 のシルクが無い。** どちらの RJ45 がどちらか基板上で判別できない。

```
J11 (y=58.5, 上側)  eth2_* = SO-DIMM 偶数ピン 16/18/24/22/28/30/36/34
J12 (y=77.0, 下側)  eth1_* = SO-DIMM 奇数ピン 15/17/23/21/27/29/35/33
```
...
どの部品をどこで買ったか間違えないように、チェックリストを生成する。

```sh
python3 tools/gen_order_xlsx.py --tag v0.9.0 --boards 2
```

`orders/<tag>/purchase-tracker.xlsx` が出る。3シート構成:

| シート | 内容 |
|---|---|
| 調達管理 | 部品を縦に並べ、状態をドロップダウンで選ぶ。行の色が自動で変わる |
| 発注先まとめ | 仕入先ごとの品目数・総個数・確認済み金額・アップロード用ファイル |
| 注意事項 | 発注前に確認すべきこと(`order_sourcing.json` の note を転記) |

状態は `未発注`(赤) / `発注済み`(黄) / `到着済み`(緑) / `手持ち在庫あり`(緑) /
`不要`(灰) から選ぶ。下部に「◯◯ / 46 品目」の進捗カウントが出る。

★**再生成しても「状態」「注文番号/日付」「備考」は引き継がれる。**
デジグネータ列をキーに既存ファイルから読み戻すので、部品構成を変えても
入力済みのチェックが消えない。

★単価が `未確認` の品目は小計に入っていない(`tools/prices.json` に無いもの)。
合計は常に下限。DigiKey 分は BOM ツールの結果を prices.json に足せば埋まる。

発注先は**どの発注CSVに載っているか**で決めている。`order_sourcing.json` の
status から引こうとすると、`status=ok` の品目は `alt_mpn` を持たないため MPN で
照合できず全部が「秋月電子等」に落ちる(2026-08-19に踏んだ)。
「digikey-bom.csv に載っている = DigiKey で買う」が定義そのものなのでそれに従う。
