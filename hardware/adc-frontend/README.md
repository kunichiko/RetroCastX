...
Blue はほぼ解消したが Green に +2.2% が残っている。Green だけは SOGIN_3 へも
1nFで分岐しており(Sync-on-Green用)、内部muxもGreenをSOGスライサへ繋いでいる。
外部HSYNC/VSYNCを使う設定(0Eh=52h)では SOG は使っていないので、SOG入力の選択を
変えるか SOGスライサを停止(2Bh bit6)すれば改善する可能性がある。ただし将来
SOG入力に対応する際に必要な経路なので、無効化するなら実行時に切り替えられる形に
すること。
