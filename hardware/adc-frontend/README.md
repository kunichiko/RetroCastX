...
測定した試作機は手半田で、**Green だけ配線が長く、AGND も遠回りしている**。
クロストークは経路の長さと戻り電流のループ面積で決まるので、これが残差の原因と
考えるのが自然。CM Offset(チャンネル間の分離改善)を有効にして改善したのに
Green だけ残る、という出方とも整合する。**基板を起こして配線長を揃えれば消える
可能性が高い**ので、この数字を基板設計の判断材料にしないこと。

なお SOGIN_3 への分岐(1nF)は基板設計には入っているが、**試作機では未配線**
なので今回の測定には効いていない。基板版で同じ症状が残る場合は、外部HSYNC/VSYNC
を使う設定(0Eh=52h)では SOG を使っていないため、SOG入力の選択変更や SOGスライサ
停止(2Bh bit6)を試す価値がある。ただし将来 SOG 入力に対応する際に必要な経路
なので、無効化するなら実行時に切り替えられる形にすること。
