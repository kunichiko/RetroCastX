...
CFG_KEY_NO_RAW_PHASE = 0x006A   # 1=生同期の位相をフィールド極性に使わない
                                #   ★生同期を繋がない方式(コンポジット/S端子/MSX)
                                #     では 1。未接続の同期入力が自己発振して
                                #     raw_ok が誤って立ち、極性が決まらなくなる
CFG_KEY_IN_MUX1 = 0x0069        # レジスタ19h [7:6]=SOG [5:4]=R [3:2]=G [1:0]=B
                                #   00=_1 / 01=_2 / 10=_3(Gのみ 11=_4)
                                #   v0.9.0 は入力ごとに系統が分かれているので全ビット
                                #   同じ値: 0x00=J5(S端子/コンポジット) /
                                #   0x55=aux 2x5(第2入力) / 0xAA=D-SUB
