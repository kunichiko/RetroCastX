...
MODES = {
    ...
    "svideo": {
        ...
        "regs": _CVBS_TIMING + [
            ...
            #
            #   1回目: 窓をバックポーチ(230)に置いたままボトムにした → バックポーチが
            #          底に張り付き、その40 IRE下の同期がクリップ。デコーダは
            #          (ブランキング-同期チップ)/40 で校正するので**輝度が5倍**になった
            #   2回目: そこで「Yはミッドレベル一択」と結論した → **これも誤り**。
            #          ミッドはブランキングが中央(164)に座るので上に91コードしか残らず、
            #          粗ゲイン最小0.5倍でも白がコード255に当たって 1.87% 飽和していた
            #
            #   正解は**窓を同期チップの中(60〜100)へ動かしてボトム**。実測:
            #
            #       ミッド/窓230/粗0.5倍    ブランク164  1 IRE=0.900  白飽和 1.87%
            #       ボトム/窓60幅40/粗0.8倍  ブランク 58  1 IRE=1.450  白飽和 0.00%
            #
            #   輝度の分解能が1.61倍になり白の飽和が消える。黒0 IRE・白100 IREは維持。
            #   窓は同期チップ(0〜127サンプル)の中に十分収めること(20幅60では壊れた)。
            #   粗ゲイン0.9倍でも飽和しないが白の余裕が13 IREしか無い。0.8倍なら34 IRE。
...
def verdict(tip, blank, burst_pp, act_max, code_per_ire, role="cvbs"):
    ...
    if role == "cvbs":
        ...
        # ★見るのは**同期チップの絶対値ではなく、ブランキングとの差**。
        #
        #   Yはボトムレベル + クランプ窓を同期チップの中(60幅40)に置くので、
        #   **同期チップが 0 付近に座っているのが正常**(実測 ブランク58 / チップ0)。
        #   デコーダは (ブランキング - 同期チップ)/40 で IRE を校正するから、
        #   必要なのは「その差が残っていること」であって、チップが0でないことでは
        #   ない。ここは2回間違えた経緯があるので MODES["svideo"] の注記も読むこと。
        #
        #   校正が壊れて輝度が数倍になったのは、窓をバックポーチ(230)に置いたまま
        #   ボトムにして**同期が下へはみ出しクリップした**とき。その形はチップの
        #   値ではなく差の消失として出るので、下の1本で捕まえられる。
        if blank - tip < 20:
            bad.append("Yの同期チップ〜ブランキングが %.0f コードしかない(40 IRE)。"
                       "**IRE校正が壊れて輝度が数倍になる。** クランプ窓が同期チップの"
                       "中に入っているか(0x1A=60 / 0x1B=40)、粗ゲイン(0x5F)が低すぎ"
                       "ないかを確認" % (blank - tip))
        if act_max >= 254:
