...
/// レジスタ37h:38h。**TVP自身が数えた1フレームのライン数。**
/// 読み出し専用。SOGスライス閾値を振ったときの合否はこれで見る
/// (15.7kHz系なら 262/263 に収束するのが正しい。閾値が低すぎて暗部を同期と
/// 誤認していると、ここが 1 になったり値が暴れたりする)。
pub const CFG_KEY_LINES_PER_FRAME: u16 = 0x0055;

/// レジスタ38h の生値。読み出し専用。**bit5 = P/I detect(0=インターレース)**。
pub const CFG_KEY_LPF_MSBS: u16 = 0x0057;

/// インターレース判定の内訳(読み出し専用、4bit)。**「織り込まれない」ときに
/// どの材料が 0 なのかを見るためのもの。** 捕捉側の `fld_pos` はこれらで決まる:
///
/// ```text
///   bit0 il_raw       生VSYNCの半ライン位相が交互か
///   bit1 raw_ok       生同期(TTL)が使える状態か
///   bit2 il_detect    TVPのP/I検出(38h bit5 の反転。1=インターレース)
///   bit3 il_frame     TVPのvtotalが2フィールドを覆っているか
/// ```
pub const CFG_KEY_STAT_IL: u16 = 0x002D;

/// SOGOUTの水平周期[pixクロック]。読み出し専用
pub const CFG_KEY_SOG_HLEN: u16 = 0x0060;

/// 生同期(TTL)の位相をフィールド極性の判定に使わない。0=使う(既定) / 1=使わない。
///
/// ★**生同期を繋がない入力方式(コンポジット/S端子/MSX)では 1 にする。**
///   未接続のシュミットバッファ入力は浮いて自己発振し、捕捉側の `raw_ok` が
///   誤って立つ。すると**存在しない同期の位相**でフィールド極性が決まり、
///   極性が交互にならないので両フィールドが同じスロットに交互に上書きされ、
///   絵が1ライン上下に震える(実機 2026-09-06、コンポジット480i:
///   `il: raw0 rawok1 P/I1 frame0` / `SOG lowmax 773` = SOG側は健全だった)。
///   **1 にして直ることを同日に実機で確認**。震えが消え、有効映像の高さが
///   217/240 → 432 になった(織り込みが成立した)。`rawok` は 1 のままなので、
///   効いたかどうかは `rawok` ではなく織り込みの成立で判断すること。
pub const CFG_KEY_NO_RAW_PHASE: u16 = 0x006A;

/// 直近で見た SOGOUT の最長Low期間[pixクロック]。読み出し専用。
///
/// ★**捕捉側の `sog_ok` はこれを使う**(`lowmax > 100 && lowmax < 0x8000`)。
///   飽和(65535)すると SOG からフィールド極性を取る経路が丸ごと落ちて、
///   両フィールドが同じスロットに書かれる。
pub const CFG_KEY_SOG_LOWMAX: u16 = 0x0061;

/// SOGOUTのLow期間を「垂直ブロードパルス」とみなす閾値[pixクロック]。既定400。
///
/// ★**既定は MSX(DATACLK 21.48MHz)で決めた値。** NTSC 8fsc(28.6356MHz)では
///   水平同期 4.7µs ≈ 135クロック / 垂直ブロードパルス 27µs ≈ 776クロックなので、
///   その間に入っていればよい。映像源で幅が違うので実行時に振れる。
pub const CFG_KEY_SOG_VTH: u16 = 0x0064;
