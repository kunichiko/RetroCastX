...
const REGS_X68000: InputRegs = &[
    ...
    //     とのことで、こちらを既定にした(×1.250 / ×1.238 / ×1.223)。
    //     校正で出したプラトーの中央(43/39/35)より一段上げた値。
    //     チャンネル間の差(R>G>B)は校正時と同じに保ってある = 緑と青の
    //     落ち込みを埋める分はそのままで、全体を持ち上げただけ。
    //     白がクリップしていないかは、階調の並んだ映像源で確認すること。
