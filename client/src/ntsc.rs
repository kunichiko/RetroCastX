...
pub struct Adjust {
    ...
    /// v^(1/g) を当てはめると **g=2.2 で9点すべてが平均誤差1.8コード**で一致。
    /// つまりあちらは符号化済みの値をリニアとして扱って**二重に符号化**した絵
    /// (washed out)。取り込み機器の色空間の扱い違いか、投稿者が暗部を見せるために
    /// 持ち上げたかまでは、動画からは分けられない。
    ///
    /// 2.2 は「YouTube と同じ見え方」であって「正しい」ではない。実用では
    /// 1.2〜1.6 あたりで好みを探るのが現実的(明るい部屋では黒が浮いて暗部が
    /// 潰れて見えるので、その対処としてのガンマ)。
    pub gamma: f32,
}
