...
class TvpCapture(Module):
    ...
    def __init__(self, pads, width=1024, height=512, nface=8, fifo_depth=4,
        ...
        # 1 で生同期の位相を使わない。既定0=生同期があれば使う(key 0x6A)。
        #
        # ★**生同期を繋がない入力方式では 1 にすること。** 未接続のシュミット
        #   バッファ入力は浮いて自己発振し、その周期が下の raw_ok の窓に入るので、
        #   **存在しない同期の位相でフィールド極性が決まってしまう**。
        #   詳細は retrocastx_stream.py の同名信号のコメント。
        ...
        if hasattr(pads, "hs_raw") and hasattr(pads, "vs_raw"):
            ...
            self.sync.pix += If(vr_edge,
                ...
                #
                # ★**「配線が無いなら周期は0か飽和値」は成り立たない。** 未接続の
                #   シュミットバッファ入力は浮いて自己発振し、周期がこの窓に入る。
                #   実機で raw_ok=1 になった(2026-09-06、コンポジット。D-SUBに
                #   何も繋いでいないのに立った)。生同期を使わない入力方式では
                #   cfg_no_raw_phase=1 で下の分岐ごと外すこと。
