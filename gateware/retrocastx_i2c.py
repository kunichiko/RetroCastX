...
class StatusDisplay(Module):
    ...
    def __init__(self, pads=None, sys_clk_freq=45e6, i2c_freq=400e3,
        ...
        #
        # ※以前の実測記録(白の90%点と飽和画素数を見ながら振ったもの):
        #     (54,51,58) 白251.5 飽和5626 / (45,42,49) 白244.5 飽和608
        #     (35,33,39) 白237.0 飽和0    ← 当時の採用値
        #   飽和は不可逆で、ノイズの山か信号の山かを区別できない。
        #   **64/61/57 は 54/51/58 より上なので、白の上端が潰れている可能性がある。**
        #   階調の並んだ映像源(x68k_calib のグレースケール)で
        #   host/python/retrocastx/calib_pattern.py を回して飽和を確認すること。
        #   見た目を採るか階調を採るかは使う人の判断なので、既定は目視の値にしてある。
