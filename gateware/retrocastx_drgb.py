...
## 生成側は**音声用の水晶**(12.288MHz)で動かす

    12.288MHz / htotal 781  →  fH = 15,733.7Hz   (実機比 -0.004%)
    vtotal 262              →  fV = 60.05Hz

★**測定側(sys 45MHz)と独立した発振器であることが肝。** 基板の 12.288MHz は
  音声ADC用の外部XOで、sys の元になる 25MHz とは別の水晶。だから:

  1. **本物のドリフトが入る。** 2つの水晶の ppm 差がそのまま出るので、
     1ライン当たりのサンプル数が実機と同じようにゆっくり動く
  2. **1ドットが 45/12.288 = 3.662 サンプル**という**分数**になる

★**2 が無いと試験にならない。** 最初は sys/3 = 15.0MHz で作っていたが、
  45/15 は**ちょうど 3.000 サンプル/ドット**。整数比だとドット境界が毎ライン
  同じ位相に来るので、**受け側の分数アキュムレータが一度も効かないまま
  「通った」ことになる**。実機は必ず分数になるので、試験の方が現実を
  再現していなかった。

★**新しいクロックドメインは作らない。** eth_rx(125MHz)のタイミングが配置シード
  次第で落ちる基板なので、クロックを増やすのは最後の手段(retrocastx_stream.py の
  --seed 参照)。既にある aud ドメインに載せるだけなので、増やしていない。

★**ドットクロックの絶対値が 14% 違っても構わない。** 受け側は同期から作り直すし、
  htotal は機種ごとの設定値として外から与えるものだから、試験で 910 でなく 781 を
...
# aud ドメイン(12.288MHz XO)をそのままドットクロックにする
DOT_HZ = 12_288_000
DOT_DIV = 1
# 12.288MHz / 781 = 15,733.7Hz。実機(14.31818MHz / 910 = 15,734.3Hz)に -0.004%
HTOTAL = 781
HACTIVE = 640
VTOTAL = 262
VACTIVE = 200
HS_WIDTH = 52      # 約4.2us。実機の値は未確認
VS_WIDTH = 3
HSTART = 96        # 後ろポーチ44 + 同期52。前ポーチは 781-96-640 = 45
VSTART = 40
# 実機の値(検算用。生成には使わない)
