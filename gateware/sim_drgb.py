...
from migen import *
from retrocastx_drgb import (DigitalRgbGen, DigitalRgbProbe, DOT_DIV, HTOTAL,
...
G = dict(htotal=96, vtotal=26, hactive=64, vactive=20,
         hstart=12, vstart=4, hs_width=6, vs_width=1, dot_div=3)
# publish 周期。小さい幾何のフレーム時間(96*26*3=7488)より十分長く取る
MEAS = 7488 * 4
POL = 7488 * 2
...
class Loop(Module):
    ...
    def __init__(self, neg_sync, row, dot):
        self.submodules.gen = gen = DigitalRgbGen(**G)
...
def run(neg_sync, row, dot, frames=6):
    ...
    res = {"bar_w": dut.bar_w}
    steps = G["htotal"] * G["vtotal"] * G["dot_div"] * frames
...
def main():
    ...
    hs_per_line = G["htotal"] * G["dot_div"]
    for neg in (1, 0):
        ...
        assert r['hlen'] == hs_per_line, f"{name}: hlen={r['hlen']} (期待 {hs_per_line})"
        # ★極性を測れていること
        ...
        assert abs(r['fh'] - want_fh) <= 1, f"{name}: fh={r['fh']} (期待 {want_fh})"
