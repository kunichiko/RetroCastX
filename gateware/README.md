...
- **MAC**: 基板のEUI-48 EEPROM(24AA025E48 ×2、0x50=ETH0/0x51=ETH1)から起動時に
  読み出す(グローバル一意)。**実装済み**(2026-09-08)。0x50 が応答しなければ 0x51 を試し、
  どちらも駄目ならローカル管理アドレス `02:52:43:58:00:01` へ落ちる。
  読み出し結果は CONFIG key 0x06〜0x08 で確認できる
- **IP**: 既定は **MAC から導いたリンクローカル `169.254.<mac[4]>.<mac[5]>`**
  (RFC 3927。第3オクテットは 1..254 に丸める)。**実装済み**(2026-09-08)。
  ★**出荷時に他人のサブネットの住所を名乗らないため。** 従来の 192.168.10.50 は
  利用者が同じ範囲を使っていれば衝突し、基板を2枚繋げば基板同士でも衝突した。
  この構成では**ホスト→ボードはブロードキャスト、ボード→ホストはSUBSCRIBEの
  送信元を学習**するので、IPに求められるのは「衝突しないこと」だけ。
  ★**IPを直接指定する運用はできなくなる**(ホストが169.254のアドレスを持たないため)。
  必要なら EEPROM の設定ページで固定IPを上書きできるようにする(フェーズ3)
- **DHCP**: LiteEth同梱のハードウェアクライアント(`liteeth/core/dhcp.py`、CPU不要)が
  使える。ただし**PC直結ではDHCPサーバがいない**ので、導入するならOSと同じく
  「DHCPを試して駄目ならリンクローカル」の二段構えが要る
